...
MATCH (i:Instructor), (d:Department)
WHERE i.instructorID = 2 AND d.departmentID = 2
CREATE (d)-[:HAS_INSTRUCTORS]->(i);

MATCH (i:Instructor), (d:Department)
WHERE i.instructorID = 3 AND d.departmentID = 3
CREATE (d)-[:HAS_INSTRUCTORS]->(i);

MATCH (i:Instructor), (d:Department)
WHERE i.instructorID = 4 AND d.departmentID = 4
CREATE (d)-[:HAS_INSTRUCTORS]->(i);
...
MATCH (s:Student), (c:Course)
WHERE s.studentID = 3 AND c.courseCode = 'BA301'
CREATE (s)-[:TAKES]->(c);

// Create relationships for student takes course (continued)
MATCH (s:Student), (c:Course)
...
// Create instructors
CREATE (:Instructor {instructorID: 1, firstName: 'Robert', lastName: 'Johnson', dateOfBirth: date('1970-05-20'), email: 'robert.johnson@example.com', educationalLevel: 'Ph.D.', departmentID: 1}),
       (:Instructor {instructorID: 2, firstName: 'Alice', lastName: 'Smith', dateOfBirth: date('1982-08-15'), email: 'alice.smith@example.com', educationalLevel: 'M.Sc.', departmentID: 2}),
       (:Instructor {instructorID: 3, firstName: 'David', lastName: 'Williams', dateOfBirth: date('1975-03-10'), email: 'david.williams@example.com', educationalLevel: 'Ph.D.', departmentID: 3}),
       (:Instructor {instructorID: 4, firstName: 'Emily', lastName: 'Davis', dateOfBirth: date('1980-11-28'), email: 'emily.davis@example.com', educationalLevel: 'M.Sc.', departmentID: 4}),
       (:Instructor {instructorID: 5, firstName: 'John', lastName: 'Miller', dateOfBirth: date('1978-02-15'), email: 'john.miller@example.com', educationalLevel: 'Ph.D.', departmentID: 5});

// Create courses
CREATE (:Course {courseCode: 'CS101', name: 'Introduction to Computer Science', creditHour: 3, description: 'Fundamentals of computer science', studyProgramID: 1}),
       (:Course {courseCode: 'EE201', name: 'Circuit Analysis', creditHour: 4, description: 'Basic principles of electrical circuits', studyProgramID: 2}),
       (:Course {courseCode: 'BA301', name: 'Marketing Strategies', creditHour: 3, description: 'Strategic marketing concepts', studyProgramID: 3}),
       (:Course {courseCode: 'BIO202', name: 'Genetics', creditHour: 4, description: 'Study of genetic principles', studyProgramID: 4}),
       (:Course {courseCode: 'PSY101', name: 'Introduction to Psychology', creditHour: 3, description: 'Overview of psychological concepts', studyProgramID: 5});

// Create students
CREATE (:Student {studentID: 1, firstName: 'John', lastName: 'Doe', dateOfBirth: date('1995-03-15'), gender: 'M', email: 'john.doe@example.com', studyProgramID: 1}),
       (:Student {studentID: 2, firstName: 'Jane', lastName: 'Smith', dateOfBirth: date('1997-08-22'), gender: 'F', email: 'jane.smith@example.com', studyProgramID: 2}),
       (:Student {studentID: 3, firstName: 'Michael', lastName: 'Johnson', dateOfBirth: date('1998-05-10'), gender: 'M', email: 'michael.johnson@example.com', studyProgramID: 1}),
       (:Student {studentID: 4, firstName: 'Emily', lastName: 'Williams', dateOfBirth: date('1996-12-03'), gender: 'F', email: 'emily.williams@example.com', studyProgramID: 3}),
       (:Student {studentID: 5, firstName: 'David', lastName: 'Brown', dateOfBirth: date('1999-02-28'), gender: 'M', email: 'david.brown@example.com', studyProgramID: 2});
